    ...
    #[requires({
            ...
            ptr::addr_eq(ptr, rebuilt_ptr)
                ...
                && kani::mem::checked_align_of_raw(ptr)
                    == Some(unsafe { Alignment::of_val_raw(rebuilt_ptr).as_usize() })
...
impl<T: ?Sized, A: Allocator> Rc<T, A> {
    ...
    // Documented caller obligation: no other Rc/Weak may reference the value.
    #[requires(Rc::strong_count(this) == 1 && Rc::weak_count(this) == 0)]
