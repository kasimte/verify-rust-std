...
mod verify {
    ...
    // `proof_for_contract` cannot resolve this jointly-generic (`T` and `A`) impl
    // method at this kani version; the contract is exercised by construction below.
    #[kani::proof]
    ...
    // `proof_for_contract` cannot resolve this jointly-generic (`T` and `A`) impl
    // method at this kani version; the contract is exercised by construction below.
    // Symbolic length is uncapped (layout-validity predicate only).
    #[kani::proof]
    ...
    fn check_drop_glue_runs() {
        ...
        kani::assume(!armed);
        kani::cover(true, "non-vacuity witness: the assumed input space is non-empty");
        drop(Box::new(PanicOnDrop(armed))); // normal drop path, no panic
    }
    ...
    fn check_drop_glue_slice_runs() {
        ...
        kani::assume(!a0 && !a1);
        kani::cover(true, "non-vacuity witness: the assumed input space is non-empty");
        let arr = [PanicOnDrop(a0), PanicOnDrop(a1)];
